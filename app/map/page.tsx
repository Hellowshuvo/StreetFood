...
export default function MapPage() {
      ...
      if (userLocation) {
        ...
        if (data && !error) {
          setStalls(data.map((s: any) => ({
            ...
            avg_rating: s.avg_rating, total_ratings: s.total_ratings,
            created_by: s.created_by, created_at: s.created_at,
            distance_meters: s.dist_meters, district_name: s.district_name,
            upazila_id: s.upazila_id, union_id: s.union_id, area_id: s.area_id,
          })));
        }
      } else if (bounds) {
        const { data, error } = await supabase.rpc('stalls_in_view', {
          p_min_lat: bounds.minLat, p_min_long: bounds.minLng,
        ...
        if (data && !error) {
          setStalls(data.map((s: any) => ({
            ...
            avg_rating: s.avg_rating, total_ratings: s.total_ratings,
            created_by: s.created_by, created_at: s.created_at,
            district_name: s.district_name, upazila_id: s.upazila_id,
            union_id: s.union_id, area_id: s.area_id,
          })));
        }
      }
    } catch (e) {
  ...
  const handleMapMove = useCallback((bounds: { minLat: number; minLng: number; maxLat: number; maxLng: number }) => {
    boundsRef.current = bounds;
    if (!userLocation) loadStalls(bounds);
  ...
  const handleLocationChange = useCallback(async (location: any) => {
    ...
    const geo = await resolveLocationCoordinates({
      ...
      upazilaId: location.upazilaId, unionId: location.unionId, areaId: location.areaId,
      upazilaName: location.upazilaName, unionName: location.unionName, areaName: location.areaName,
    });
    if (geo) setMapView({ center: [geo.lat, geo.lng], zoom: geo.zoom });
  }, []);
  ...
  const filteredStalls = stalls
    ...
    .sort((a, b) => {
      if (sortMode === 'top-rated') return Number(b.avg_rating) - Number(a.avg_rating);
      if (sortMode === 'nearby' && a.distance_meters && b.distance_meters) return a.distance_meters - b.distance_meters;
      return 0;
    });
  ...
  return (
    <div className={styles.layout}>
      <main className={styles.mapArea}>
        ...
        <div className={styles.mobileHeader}>
          <div className={styles.mobileBrand}>
            <div className={styles.mobileHeaderLeft}>
              <button className={styles.mobileMenuBtn} onClick={() => setIsSidebarOpen(true)} aria-label="Open menu">
        ...
        {!isLocationActive && (
          <button className={styles.fab} onClick={handleAddClick} aria-label="Add stall">
      ...
      <Sidebar
        ...
        activePage="map"
        onPageChange={(page) => { if (page === 'feed') router.push('/'); }}
